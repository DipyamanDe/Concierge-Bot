...
const StaffCard = ({ data,onEditClick }) => {
    const statusText = data.Staff_Status ? "Active" : "Inactive";
    return (
        <div className="m-10 p-3 border-2 w-[500px] h-[125px] flex flex-row justify-center rounded-lg bg-slate-500 hover:bg-slate-700  text-gray-100">
            <div className="w-1/4 h-full flex flex-col ">
                <div className="text-xs h-[20%] flex justify-center">Staff Id: {data.Staff_Id}</div>
                <div className="text-5xl h-[80%] flex justify-center items-center"><FontAwesomeIcon icon={faUser} size="lg" /></div>
            </div>
            <div className="w-3/4 flex flex-col justify-between p-1 overflow:hidden">
                <div className="h-[30%] text-xl">{data.Staff_Name}</div>
                <div className="flex flex-row justify-between  h-[40%] ">
                    <div className="flex flex-col text-xs justify-evenly ">
                        <div className={`text-${data.Staff_Status ? 'green' : 'red'}`}>Status: {statusText}</div>
                        <div className="">Email: {data.Staff_email}</div>
                    </div>
                    <div className="flex flex-col pr-3"> <div className=" text-xs">Dept: {data.Staff_Dept}</div>
                    </div>
                </div>
