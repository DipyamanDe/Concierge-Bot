...
const ReservationCard = ({ data , onEditClick}) => {
    const statusText = data.Reservation_Status ? "Reserved" : "Un Reserved";
    return (
        <div className="m-10 p-3 border-2 w-[500px] h-[125px] flex flex-row justify-center rounded-lg bg-slate-500 hover:bg-slate-700  text-gray-100">
            <div className="w-1/4 h-full flex flex-col ">
                <div className="text-xs h-[20%] flex justify-center">Reservation Id: </div>
                <div className="text-6xl h-[80%] flex justify-center items-center">{data.Reservation_Id}</div>
                
            </div>
            <div className="w-3/4 flex flex-col justify-between p-1 overflow:hidden">
                <div className="h-[30%] text-xl"> Reservation Type :- {data.Reservation_Type}</div>
                <div className="flex flex-row justify-between  h-[40%] ">
                    <div className="flex flex-col text-xs justify-evenly ">
                        <div className=""></div>
                        <div className=""></div>
                    </div>
                    <div className="flex flex-col pr-3 text-xs"> 
                        <div className={`text-${data.Reservation_Status ? 'green' : 'red'}`}>Reservation Status:- {statusText}</div>
                        <div className=" text-xs">Reservation Description:-  {data.Reservation_Description}</div>
                    </div>
                </div>
