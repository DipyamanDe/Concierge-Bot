...
function ChatMainPage() {
  ...
  const sendMessage = async (assistant_message) => {
    ...
    delayToShowContent(response);

  //   console.log("response-->",typeof(response.data.assistant_message))
  //   if (response.data) {
  ...
  //     delayToShowContent(response);
  // } else {
  //     console.error("Error: Unable to extract message content from response");
  // }
  ...
  const delayToShowContent = (response) => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      if(usingmic){
      play(response.data.assistant_message);}
    }, 1000);
            ...
            >
              {message.role === 'user' ?(
                ...
                <div className='flex flex-col'>
                  ...
                  <div className='flex-end ' style={{ justifySelf: 'end' }}>
                  {
                    index===playingIndex && isPlaying ? (
                    ...
                    ):(<div  onClick={() => playAudio(message.content, index)}>
