...
const RoomCardEdit = ({ data, onSaveClick, onCancelClick }) => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-gray-900 bg-opacity-75">
      <div className="bg-white p-6 rounded-lg">
        <h2 className="block text-xl font-bold text-black-700 mb-2">Edit Room</h2>
        <form>
          <div className="mb-4">
            <label htmlFor="Room_Price" className="block text-sm font-medium text-gray-700">Room Price</label>
            <input
              type="text"
              id="Room_Price"
              value={editedData.Room_Price}
              onChange={(e) => setEditedData({ ...editedData, Room_Price: e.target.value })}
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={(e) => setEditedData({ ...editedData, Room_Type: e.target.value })}
              className="mt-1 p-2 w-full border rounded-md"
            />
          </div>
        </form>
